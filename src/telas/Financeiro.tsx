...
function FinConsultor() {
  ...
  if (souGestor && !alvo) {
    ...
    return (
      <section className="view active" id="view-financeiro">{cab}
        <div className="kpis" id="finKpis"><Kpi n={totVisitas} l="Visitas pagas" /><Kpi n={totVendas} l="Vendas efetivadas" /><Kpi n={fmtMoeda(totVendido)} l="Valor vendido" /><Kpi n={fmtMoeda(totComissao)} l={`Comissão (${pct}%)`} /><Kpi n={fmtMoeda(totVisitasR + totComissao)} l="Total a receber (consultores)" cor="var(--st-concluida)" /></div>
        <div id="finResumoTodos" className="panel"><h3>Por consultor <span className="hint" style={{ marginLeft: 6 }}>clique para ver o extrato</span></h3>
          {resumos.length ? <div style={{ overflowX: "auto" }}><table className="dl-tab"><thead><tr><th>Consultor</th><th>Visitas pagas</th><th>Vendas efetivadas</th><th>Valor vendido</th><th>Pagamento por visitas</th><th>Comissão ({pct}%)</th><th>Valor a receber</th></tr></thead>
            <tbody>{resumos.sort((a, b) => b.r.total - a.r.total || b.r.totalVendido - a.r.totalVendido).map(({ u, r }) => (
              <tr key={u.id} onClick={() => setCons(u.id)} style={{ cursor: "pointer" }}><td><b>{u.nome}</b></td><td>{r.visitas.length}</td><td>{r.vendas.length}</td><td style={{ fontWeight: 700 }}>{fmtMoeda(r.totalVendido)}</td><td>{fmtMoeda(r.pagamentoVisitas)}</td><td>{fmtMoeda(r.comissao)}</td><td style={{ fontWeight: 800, color: "var(--st-concluida)" }}>{fmtMoeda(r.total)}</td></tr>))}</tbody>
            <tfoot><tr><td><b>Total</b></td><td><b>{totVisitas}</b></td><td><b>{totVendas}</b></td><td><b>{fmtMoeda(totVendido)}</b></td><td><b>{fmtMoeda(totVisitasR)}</b></td><td><b>{fmtMoeda(totComissao)}</b></td><td style={{ fontWeight: 800, color: "var(--st-concluida)" }}>{fmtMoeda(totVisitasR + totComissao)}</td></tr></tfoot></table></div>
            : <div className="empty">Nenhum consultor cadastrado.</div>}
          <div style={{ fontSize: 12, color: "var(--ink-faint)", marginTop: 8 }}>Valor a receber = pagamento por visitas realizadas (R$ {pagamentoVisita} cada) + comissão de {pct}% sobre as vendas efetivadas. Vendas a confirmar e promissórias não entram.</div>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="view active" id="view-financeiro">{cab}
      <div className="kpis" id="finKpis"><Kpi n={r.visitas.length} l="Visitas pagas" /><Kpi n={fmtMoeda(r.pagamentoVisitas)} l="Pagamento por visitas" /><Kpi n={r.vendas.length} l="Vendas efetivadas" /><Kpi n={fmtMoeda(r.totalVendido)} l="Valor vendido" /><Kpi n={fmtMoeda(r.comissao)} l={`Comissão (${pct}%)`} /><Kpi n={fmtMoeda(r.total)} l="Valor a receber" cor="var(--st-concluida)" /></div>
