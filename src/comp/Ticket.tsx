...
import { STATUS, STATUS_CLIENTE, diasRestantes, estaAtrasado, fmtDate, fmtDateTime, fmtDiaHora, situacaoPrazo } from "../lib/regras";
...
export function Ticket({ c, resposta }: { c: any; resposta?: boolean }) {
  ...
  const late = estaAtrasado(c), prazo = situacaoPrazo(c), st = STATUS[c.status], dr = diasRestantes(c);
  const presale = R.domMarketing(c);
  const slaTxt = (c.status === "respondida" || c.status === "concluida") ? "Respondido" : prazo === "critico" ? "Crítico — sem resposta há +24h" : late ? "Atrasado" : (dr <= 0 ? "Vence hoje" : `Responder em ${dr} ${dr === 1 ? "dia" : "dias"}`);
  const slaCls = prazo === "critico" ? "critico" : prazo === "atrasado" ? "late" : prazo === "perto" ? "warn" : "";
  const critMkt = presale && R.clienteCriticoInatividade(c);
  const urg = presale
    ? (critMkt ? <span className="badge b-critico" title={Math.floor(R.horasSemAtualizar(c)) + "h sem atualização"}>🔴 Crítico — sem atualização</span> : null)
    : (prazo === "critico" ? <span className="badge b-critico">🔴 Crítico</span> : (c.urgente && c.status !== "concluida" ? <span className="badge b-urgente">⚠ Urgente</span> : null));
  ...
  return (
    <div className="ticket" data-id={c.id} data-prazo={prazo} onClick={() => abrirDetalhe(c.id)}>
