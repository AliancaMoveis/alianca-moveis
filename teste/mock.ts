...
const usuarios = [
  ...
  [8, "Fernanda Melo", ["checklist"], false], [9, "Igor Tavares", ["medidas"], true], [19, "Lucilene", ["medidas_supervisao", "medidas"], false], [10, "Ana Ribeiro", ["marketing_operadora"], false],
...
const setores = [
  ...
  ["checklist", "Checklist", L()], ["medidas", "Medidas", L()], ["medidas_supervisao", "Supervisão de Medidas", L(true)], ["marketing_operadora", "Operadora Marketing", L(true)],
...
export function estadoMock(): Estado {
  return {
    ...
    chamados: (seed as any[]).concat(POSVENDA).concat(MEDIDAS).map(c => ({
      ...
      escalonadoAuto: false, escaladoEm: null, consultorId: c.con || "", atendenteId: c.ate || "",
      dataVisita: local(c.vis), endereco: c.end, medidorId: c.medi || "", dataMedida: local(c.dm ?? null), vinculadoA: c.vinc || null, dataLoja: local(c.loja), statusCliente: c.sc || "", tratativa: c.tr,
